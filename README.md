...
- 4단계 `Reaction Lab`: 탄화수소 종류, 탄소 수, 시약을 함께 조합해 분자 계산, 연소식, 시약 반응을 한 화면에서 확인합니다.
...
- 실제 화학에서는 암모니아성 질산은 반응이 말단 알카인에 더 정확하지만, 이 앱은 과제 원본 Python 로직을 기준으로 알카인 반응을 단순화해 표시합니다.
...
- 분자식, 분자량, 산소 요구량, 생성물 수, 연소식, 시약 반응 결과가 함께 갱신됩니다.
- 선택된 객체가 `HydrocarbonADT -> Hydrocarbon -> Concrete Class` 흐름으로 어떻게 동작하는지 표시합니다.
